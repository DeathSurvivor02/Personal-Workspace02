...
public class Atom
{
	...
	// Parse method to convert a string to an Atom object
	...
	// Method to return a string representation of an Atom object
	public override string ToString()
	{
		return $"{Name} ({Symbol}) - Protons: {Proton}, Neutrons: {Neutron}, Weight: {Weight}";
	}

	// Static method to get a list of atoms
	...
	// Method to serialize the list of atoms to a JSON file
	public static void WriteJson(List<Atom> atoms, string filename)
	{
		var json = JsonSerializer.Serialize(atoms);
		File.WriteAllText(filename, json);
	}

	// Method to deserialize atoms from a JSON file
	public static void ReadJson(string filename)
	{
		var json = File.ReadAllText(filename);
		var atoms = JsonSerializer.Deserialize<List<Atom>>(json);
		atoms.ForEach(atom => Console.WriteLine(atom.ToString()));
	}

	// Method to serialize the list of atoms to an XML file
	public static void WriteXml(List<Atom> atoms, string filename)
	{
		var serializer = new XmlSerializer(typeof(List<Atom>));
	...
	// Method to deserialize atoms from an XML file
	public static void ReadXml(string filename)
	{
		var serializer = new XmlSerializer(typeof(List<Atom>));
		using (var reader = new StreamReader(filename))
		{
			var atoms = (List<Atom>)serializer.Deserialize(reader);
			atoms.ForEach(atom => Console.WriteLine(atom.ToString()));
		}
	}
}
...
class Program
{
	static void Main()
	{
		...
		// Display all atoms
		Console.WriteLine("Displaying all atoms:");
		atoms.ForEach(atom => Console.WriteLine(atom.ToString()));

		// Serialize the first atom using JSON
		Console.WriteLine("\nSerializing the first atom to JSON...");
		Atom.WriteJson(new List<Atom> { atoms[0] }, "atom.json");

		// Read the first atom from JSON
		Console.WriteLine("\nReading the first atom from JSON...");
		Atom.ReadJson("atom.json");

		// Serialize the entire collection to JSON
		Console.WriteLine("\nSerializing all atoms to JSON...");
		Atom.WriteJson(atoms, "atoms.json");

		// Read the entire collection from JSON
		Console.WriteLine("\nReading all atoms from JSON...");
		Atom.ReadJson("atoms.json");

		// Serialize the first atom using XML
		Console.WriteLine("\nSerializing the first atom to XML...");
		Atom.WriteXml(new List<Atom> { atoms[0] }, "atom.xml");

		// Read the first atom from XML
		Console.WriteLine("\nReading the first atom from XML...");
		Atom.ReadXml("atom.xml");

		// Serialize the entire collection to XML
		Console.WriteLine("\nSerializing all atoms to XML...");
		Atom.WriteXml(atoms, "atoms.xml");

		// Read the entire collection from XML
		Console.WriteLine("\nReading all atoms from XML...");
		Atom.ReadXml("atoms.xml");
	}
}

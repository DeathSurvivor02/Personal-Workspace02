...
ProjectFrame::ProjectFrame ( const wxString& title, const wxPoint& pos, const wxSize& size) : wxFrame((wxFrame *)NULL, -1, title, pos, size)
{
    ...
    SetStatusText( wxT(" Faith Waithe"), 1);

    //third section of the status bar
    SetStatusText( wxT(" 400011003"), 2);
...
void ProjectFrame::OnOpenFile(wxCommandEvent& event)
{
    ...
    if (OpenDialog->ShowModal() == wxID_OK)     //if the user click "Open" instead of "canel"
    {
        ...
        //set the title
        SetTitle(wxString(wxT("COMP2611 - Data STructures : 400011003")));
    }
}
...
void ProjectFrame::OnAbout(wxCommandEvent& event)
{
    ...
    wxMessageBox(wxT("Faith Waithe\nTop #1 Billboard Songs for 1990s\nLinux OpenSuse Leap 15.4"), wxT("About"), wxOK | wxICON_INFORMATION, this);
}
